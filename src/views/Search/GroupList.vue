  ...
  <div>
    ...
    <b-row class="table-controls">
      <b-col cols="4" class="show-quantity">
        ...
        {{ $t("general.of") }} {{ total }} {{ $t("group.label") }}
        {{ $t("general.results") }}
      </b-col>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        {
          key: "related_family_title",
          label: "Related Family Title",
          sortable: true
        },
        { key: "brands", label: "Brands", sortable: true }
      ],
      currentPage: 1,
      selected: null,
      search_term: "",
  ...
  errorCaptured: function(err, component, info) {
    console.error("GroupList Error", component, info);
    console.error(err);
  ...
  methods: {
    searchGroups: async function() {
      ...
      try {
        ...
        let apiresp = await Vue.mtapi.searchGroups(query);
    ...
    async exportCurrentResults() {
      ...
      try {
        ...
        const res = await Vue.mtapi.getExportGroups(query);
        helpers.exportCSVFile(res.headers[0], res.groups, "groups");
      } catch (err) {
        if (err instanceof ApiError) {
          this.error = `Couldn't get groups. ${err.message}`;
        } else {
    ...
    async exportAllResults() {
      let res = await Vue.mtapi.exportTable("group");
      helpers.exportCSVFile(res.headers[0], res["groups"], "all_groups");
    }
  }
};
</script>
